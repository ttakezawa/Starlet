...
sub setup_listener {
    ...
    for (my ($fd, $info) = each %{$self->{fd_info}}) {
        my $family = Socket::sockaddr_family(getsockname($info->{listen_sock}));
        $info->{_listen_sock_is_tcp} = $family != AF_UNIX;

        $info->{listen_sock}->blocking(0);

        # set defer accept
        if ($^O eq 'linux' && $info->{_listen_sock_is_tcp}) {
            setsockopt($info->{listen_sock}, IPPROTO_TCP, 9, 1)
                and $info->{_using_defer_accept} = 1;
        }
    }
...
sub accept_loop {
    ...
    my $fd_info = $self->{fd_info};
    while (my ($fd, $info) = each %$fd_info) {
        $select->add($info->{listen_sock});
    }

    while (! defined $max_reqs_per_child || $proc_req_count < $max_reqs_per_child) {
        for my $listen_sock ($select->can_read()) {
            my $info = $fd_info->{$listen_sock->fileno};
            if (my ($conn,$peer) = $listen_sock->accept) {
                $info->{_is_deferred_accept} = $info->{_using_defer_accept};
                $conn->blocking(0)
                    or die "failed to set socket to nonblocking mode:$!";
                my ($peerport, $peerhost, $peeraddr) = (0, undef, undef);
                if ($info->{_listen_sock_is_tcp}) {
                ...
                while (1) {
                    ...
                    ++$proc_req_count;
                    my $env = {
                        SERVER_PORT => $info->{port} || 0,
                        SERVER_NAME => $info->{host} || 0,
                        SCRIPT_NAME => '',
